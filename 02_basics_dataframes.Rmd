...
```{r}
#which.min(airquality$Temp)  #<- index only
...
```{r}
dasauRus <- data.frame(datasaurus_dozen)
```

```{r}
table(dasauRus$dataset)
```
...
```{r}
if (!require(ggplot2)) install.packages('ggplot2', dependencies = T) # to load dataset msleep
library(ggplot2)
...
```{r}
# No need to use with and |> for this:

# Calculate the correlation between sleep_total and sleep_rem
correlation <- cor(msleep$sleep_total, msleep$sleep_rem, use = "complete.obs")
