...
There appears to be a decreasing trend in the plot. 
To aid the eye, we can add a smoothed line by adding a new geom, **geom_smooth**, to the figure:
...
By default, geom_smooth adds a line fitted using either LOESS25 or GAM26, as well as the corresponding 95% confidence interval
describing the uncertainty in the estimate. There are several useful arguments that can be used with geom_smooth. 
...
(Loess short for Local Regression)

To decrease the smoothness of the line, use the **span** argument in geom_smooth. 
The default is geom_smooth(span = 0.75). Decreasing this values yields a very different fit:
