...
- Outliers: observations that deviate too much11 from the rest are shown as separate points. 
...
```{r}
ggplot(penguins, aes(species, bill_length_mm)) +
      geom_boxplot(colour = "brown", fill = "lightblue")
```
...
```{r}
ggplot(penguins, aes(species, bill_length_mm)) +
      geom_boxplot(colour = "brown", fill = "lightgreen") +
      geom_jitter(size = 0.75, alpha = 0.75, height = 0.5)
```

**geom_jitter can be used to plot the individual observations on top of the histogram.**
It is often a good idea to set a small size and a low alpha in order not to cover the boxes completely.
...
To create a stacked bar chart using ggplot2:
...
ggplot(penguins, aes(species)) +
      labs(x = "Species", y = "Count") +
      geom_bar(fill = c("red", "green", "blue"), width = 0.5)
```
